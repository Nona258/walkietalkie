...
interface AdminNavbarProps {
  activeTab: 'dashboard' | 'siteManagement' | 'walkieTalkie' | 'activityLogs' | 'companyList' | 'employee' | 'settings' | 'technicalSupport' | 'employeeLogs';
  setActiveTab: (tab: 'dashboard' | 'siteManagement' | 'walkieTalkie' | 'activityLogs' | 'companyList' | 'employee' | 'settings' | 'technicalSupport' | 'employeeLogs') => void;
  onNavigate?: (page: string) => void;
  onLogout: () => void;
  pendingUsersCount?: number;
  // Legacy prop names (isMobileOpen/onMobileClose) and newer names
  // (`isMobileMenuOpen`/`setIsMobileMenuOpen`) are both supported for
...
export default function AdminNavbar({ activeTab, setActiveTab, onNavigate, onLogout, pendingUsersCount, isMobileOpen = false, onMobileClose, isMobileMenuOpen, setIsMobileMenuOpen }: AdminNavbarProps) {
    ...
    <>
      ...
      >
        ...
        <ScrollView className={`flex-1 ${isExpanded ? 'px-3' : 'px-2 pt-3'}`} showsVerticalScrollIndicator={false}>
          ...
          {/* Employee Logs */}
          <TouchableOpacity
            onPress={() => { if (onNavigate) onNavigate('EmployeeLogs'); }}
            activeOpacity={0.7}
            className={`flex-row items-center py-2.5 mb-0.5 rounded-lg ${isExpanded ? 'px-3' : 'px-0 justify-center'}`}
          >
            <View className={`w-8 h-8 rounded-lg items-center justify-center ${isExpanded ? 'mr-3' : ''} bg-transparent`}>
              <Ionicons name="document-text-outline" size={18} color={"#a8a29e"} />
            </View>
            {isExpanded && (
              <Text className={`text-sm font-medium flex-1 text-stone-500`}>Employee Logs</Text>
            )}
          </TouchableOpacity>

          {/* Technical Support */}
          <TouchableOpacity
            onPress={() => { if (onNavigate) onNavigate('TechnicalSupport'); }}
            activeOpacity={0.7}
            className={`flex-row items-center py-2.5 mb-0.5 rounded-lg ${isExpanded ? 'px-3' : 'px-0 justify-center'}`}
          >
            <View className={`w-8 h-8 rounded-lg items-center justify-center ${isExpanded ? 'mr-3' : ''} bg-transparent`}>
              <Ionicons name="help-circle-outline" size={18} color={"#a8a29e"} />
            </View>
            {isExpanded && (
              <Text className={`text-sm font-medium flex-1 text-stone-500`}>Technical Support</Text>
            )}
          </TouchableOpacity>
        ...
        <View className={`pb-5 pt-3 border-t border-stone-100 ${isExpanded ? 'px-3' : 'px-2'}`}>
          <View className={`bg-stone-50 rounded-xl py-3 mb-2 flex-row items-center ${isExpanded ? 'px-3' : 'px-0 justify-center'}`}>
            <View className={`w-8 h-8 rounded-full items-center justify-center bg-[#237227] ${isExpanded ? 'mr-3' : ''}`}>
          ...
          <Pressable onPress={e => e.stopPropagation()}>
            <View className="w-[270px] flex-1 bg-[#f8fafb] border-r border-stone-200">
              ...
              <ScrollView className="flex-1 px-3 pt-2" showsVerticalScrollIndicator={false}>
                {NAV_ITEMS.map((item) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      activeOpacity={0.7}
                      className={`flex-row items-center py-2.5 px-3 mb-0.5 rounded-lg ${isActive ? 'bg-[#e8f5e9]' : 'bg-transparent'}`}
                    >
                      {isActive && (
                        <View className="absolute left-0 top-2 bottom-2 w-0.5 rounded bg-[#237227]" />
                      )}
                      <View className={`w-8 h-8 rounded-lg items-center justify-center mr-3 ${isActive ? 'bg-[#f8fafb]' : 'bg-transparent'}`}>
                        <Ionicons
                          name={(isActive ? item.activeIcon : item.icon) as any}
                          size={18}
                          color={isActive ? '#237227' : '#a8a29e'}
                      ...
                      <Text className={`text-sm font-medium flex-1 ${isActive ? 'text-[#237227]' : 'text-stone-500'}`}>
                        {item.label}
                      </Text>
                    </TouchableOpacity>
                  );
                ...
                {/* Employee Logs (mobile) */}
                <TouchableOpacity
                  onPress={() => { if (onNavigate) { onNavigate('EmployeeLogs'); handleMobileClose(); } }}
                  activeOpacity={0.7}
                  className="flex-row items-center py-2.5 px-3 mb-0.5 rounded-lg bg-transparent"
                >
                  <View className="items-center justify-center w-8 h-8 mr-3 rounded-lg">
                    <Ionicons name="document-text-outline" size={18} color={'#a8a29e'} />
                  </View>
                  <Text className="flex-1 text-sm font-medium text-stone-500">Employee Logs</Text>
                </TouchableOpacity>

                {/* Technical Support (mobile) */}
                <TouchableOpacity
                  onPress={() => { if (onNavigate) { onNavigate('TechnicalSupport'); handleMobileClose(); } }}
                  activeOpacity={0.7}
                  className="flex-row items-center py-2.5 px-3 mb-0.5 rounded-lg bg-transparent"
                >
                  <View className="items-center justify-center w-8 h-8 mr-3 rounded-lg">
                  ...
                  <Text className="flex-1 text-sm font-medium text-stone-500">Technical Support</Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                  className={`flex-row items-center py-2.5 px-3 mb-0.5 rounded-lg ${activeTab === 'settings' ? 'bg-[#e8f5e9]' : 'bg-transparent'}`}
                >
                  {activeTab === 'settings' && (
                    <View className="absolute left-0 top-2 bottom-2 w-0.5 rounded bg-[#237227]" />
                  )}
                  <View className={`w-8 h-8 rounded-lg items-center justify-center mr-3 ${activeTab === 'settings' ? 'bg-[#f8fafb]' : 'bg-transparent'}`}>
                    <Ionicons
                      name={activeTab === 'settings' ? 'settings' : 'settings-outline'}
                      size={18}
                      color={activeTab === 'settings' ? '#237227' : '#a8a29e'}
                  ...
                  <Text className={`text-sm font-medium flex-1 ${activeTab === 'settings' ? 'text-[#237227]' : 'text-stone-500'}`}>
                    Settings
                  </Text>
                </TouchableOpacity>
              </ScrollView>
