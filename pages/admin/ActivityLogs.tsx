  ...
  useEffect(() => {
    ...
    if (activeFilter !== 'All') {
      filtered = filtered.filter(activity => {
        switch (activeFilter) {
          case 'Group':
            return activity.type === 'group' || activity.action.toLowerCase().includes('group');
          case 'Site':
            return activity.type === 'site' || activity.action.toLowerCase().includes('site');
          case 'Contacts':
            return activity.type === 'contacts' || activity.action.toLowerCase().includes('contact');
          default:
            return true;
        }
      });
            ...
            <View className="px-5 py-3 border-b border-stone-100 lg:px-6">
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                <View className="flex-row gap-2">
                  {['All', 'Group', 'Site', 'Contacts'].map((filter) => (
                    <TouchableOpacity
                      key={filter}
