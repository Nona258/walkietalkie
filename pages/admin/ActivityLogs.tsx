            ...
            <View className="px-3 lg:px-6 py-3 lg:py-4">
              {loading ? (
                ...
                activities.map((activity, index) => (
                  <View key={activity.id} className={`flex-row items-start py-4 lg:py-5 ${index !== activities.length - 1 ? 'border-b border-stone-100' : ''}`}>
                    <View className="w-12 h-12 lg:w-14 lg:h-14 rounded-xl lg:rounded-2xl items-center justify-center mr-3 lg:mr-4" style={{ backgroundColor: activity.color || '#ecfdf5' }}>
                      <Ionicons name={(activity.icon || 'notifications-outline') as any} size={20} color="#10b981" />
                    </View>
                    <View className="flex-1">
                    ...
                    <Text className="text-xs lg:text-sm text-stone-400 ml-3 lg:ml-4 mt-1">{activity.time ? new Date(activity.time).toLocaleString() : ''}</Text>
                  </View>
                ))
              )}
            </View>
