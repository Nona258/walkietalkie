...
export default function EmployeeLogs({ onBack, isMobileMenuOpen, setIsMobileMenuOpen }: EmployeeLogsProps) {
    ...
    <View className="flex-1 bg-stone-50">
      <ScrollView
        ...
        <View className="px-6 pt-5 pb-4 bg-white border-b border-stone-100">
          <View className="flex-row items-center justify-between">
            <View className="flex-row items-center">
              ...
              {onBack && (
                <TouchableOpacity
                  className="items-center justify-center mr-3 border rounded-lg h-9 w-9 border-stone-100 bg-stone-50"
                  onPress={onBack}
                  activeOpacity={0.8}>
        ...
        <View className="px-6 pt-4 pb-6">
          {loading ? (
            <View className="items-center justify-center p-6 bg-white border rounded-xl border-stone-100">
              <ActivityIndicator size="small" color="#10b981" />
              <Text className="mt-3 text-sm text-stone-500">Loading attendance…</Text>
            </View>
          ) : error ? (
            <View className="p-6 border border-red-100 rounded-xl bg-red-50">
              <Text className="text-sm font-semibold text-red-600">Error: {error}</Text>
              <TouchableOpacity
                className="flex-row items-center self-start px-3 py-2 mt-4 bg-white rounded-lg"
                onPress={fetchRows}
                activeOpacity={0.8}>
            ...
            <View className="items-center justify-center p-8 bg-white border rounded-xl border-stone-100">
            ...
            <View className="gap-3">
              {rows.map((r) => (
                <View
                  key={String(r.id)}
                  className="p-4 bg-white border rounded-xl border-stone-100"
                  style={{
                    shadowColor: '#000',
                  ...
                  <View className="flex-row items-start justify-between">
                    ...
                    <View className="px-2 py-1 rounded-full bg-stone-100">
