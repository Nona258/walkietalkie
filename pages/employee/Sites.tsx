...
export default function Sites({ onMapPress }: SitesProps) {
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      {/* Header Section */}
      <View className="bg-white px-6 py-6 pt-12">
        <View className="flex-row items-center justify-between mb-6">
          <View>
            <Text className="text-black text-3xl font-extrabold">Sites</Text>
            <Text className="text-xs font-semibold mt-1" style={{ color: '#237227' }}>Manage your locations</Text>
          </View>
          <TouchableOpacity onPress={onMapPress} className="rounded-full p-3 active:scale-95" style={{ backgroundColor: '#237227' }}>
            <Ionicons name="map" size={24} color="white" />
          </TouchableOpacity>
        </View>

        {/* Search Bar */}
        <View className="flex-row items-center rounded-2xl bg-gray-100 px-4 py-3 border-2" style={{ borderColor: searchText.length > 0 ? '#237227' : '#237227' }}>
        ...
        <View className="px-6 py-6 pb-28">
          {loading ? (
            <View className="items-center justify-center py-16">
              <Text className="text-base mt-4 font-semibold" style={{ color: '#237227' }}>Loading sites...</Text>
            </View>
            ...
            <View className="gap-4">
              {filteredSites.map((site) => (
                <TouchableOpacity
                  ...
                  className="rounded-3xl p-5 active:scale-95"
                  style={{ backgroundColor: '#e8f5e9' }}
                >
                  {/* Top Section - Icon and Basic Info */}
                  <View className="flex-row items-start mb-5">
                    {/* Location Icon */}
                    <View className="w-14 h-14 rounded-2xl items-center justify-center mr-4" style={{ backgroundColor: site.status === 'active' ? '#237227' : '#9ca3af' }}>
                    ...
                    <View className="flex-1">
                      <View className="flex-row items-center mb-1">
                        <Text className="text-black font-extrabold text-lg flex-1">
                          {site.name}
                        </Text>
                        <View className="h-3 w-3 rounded-full" style={{ backgroundColor: site.status === 'active' ? '#237227' : '#9ca3af' }} />
                      </View>
                  ...
                  {/* Staff Count Section */}
                  <View className="rounded-2xl p-4 items-center justify-center mb-4" style={{ backgroundColor: '#f8fafb' }}>
                    <Text className="text-xs font-semibold uppercase" style={{ color: '#6b7280' }}>Staff Count</Text>
                    <Text className="font-extrabold text-2xl mt-2" style={{ color: '#237227' }}>{site.staffCount}</Text>
                  </View>
                  ...
                  <View className="flex-row items-center">
                    <Ionicons name="home-outline" size={16} color="#6b7280" />
                      ...
                      {site.branchName && (
                        <Text className="text-sm ml-2 flex-1 font-semibold mt-0.5" style={{ color: '#237227' }} numberOfLines={1}>
                          {site.branchName}
                        </Text>
                      )}
                    <Ionicons name="chevron-forward" size={20} color="#237227" />
                  </View>
                </TouchableOpacity>
              ))}
            </View>
